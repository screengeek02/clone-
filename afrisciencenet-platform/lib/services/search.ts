...
export async function runGlobalSearch(query: string) {
  const q = query.trim();
  if (!q) return { equipment: [], researchers: [], funding: [], institutions: [], collaborations: [] };

  const [equipment, researchers, funding, institutions, collaborations] = await Promise.all([
  ...
  return { equipment, researchers, funding, institutions, collaborations };
}
